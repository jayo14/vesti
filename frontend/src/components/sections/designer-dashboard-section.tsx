...
import {
  Package, ShoppingCart, Wallet, Plus, Edit3, Trash2, Loader2, TrendingUp, Image, AlertCircle
} from "lucide-react";
...
export function DesignerDashboardSection() {
  ...
  const [newProduct, setNewProduct] = useState({ name: "", description: "", price: "", stock: "1", category: "", image_url: "", material: "", fit_type: "", is_published: false });
  ...
  const startEdit = (p: any) => {
    setEditingProduct(p);
    setNewProduct({ name: p.name, description: p.description || "", price: p.price.toString(), stock: p.stock.toString(), category: p.category_id?.toString() || "", image_url: p.image_url || "", material: p.material || "", fit_type: p.fit_type || "", is_published: p.is_published });
    setUploadError("");
    setShowForm(true);
  };
  ...
  const saveProduct = async () => {
    ...
    try {
      ...
      if (r.ok) {
        ...
        setNewProduct({ name: "", description: "", price: "", stock: "1", category: "", image_url: "", material: "", fit_type: "", is_published: false });
        setFormErrors({});
        fetchDash();
      } else {
        const d = await r.json().catch(() => ({}));
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <button                 onClick={() => { setShowForm(true); setEditingProduct(null); setNewProduct({ name: "", description: "", price: "", stock: "1", category: "", image_url: "", material: "", fit_type: "", is_published: false }); setFormErrors({}); setUploadError(""); }}
              ...
              <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <FieldBlock label="Material" error={formErrors.material}>
                      <select value={newProduct.material} onChange={(e) => setNewProduct({ ...newProduct, material: e.target.value })}
                        ...
                        {options.materials.map((m) => <option key={m.value} value={m.value}>{m.label}</option>)}
                      </select>
                    </FieldBlock>
                  ...
                  <div className="p-3 rounded-xl bg-amber-500/10 border border-amber-500/20 text-xs text-amber-700">
                    New products start as <strong>Draft</strong>. After saving, you can submit for admin review.
                  </div>
                    ...
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      ...
                      {(p.tryons_total || 0) > 0 && (
                        <span className="text-[10px] text-muted-foreground/60 shrink-0">
                          {p.tryons_total} try-ons
                          {p.tryons_total > 3 && p.orders_count === 0 ? " — no purchases" : ""}
                        </span>
                      )}
                    </div>
